...
export default handleActions({
  ...
  [DELETE_LAYER]: (state, {payload}) => {
    ...
    let layers, panel, panels
    switch (type) {
      case 'data':
        layers = [...state.panels[activeMapId].layers]
        layers.splice(idx, 1)
        panel = {...state.panels[activeMapId], layers: layers}
        panels = [...state.panels]
  ...
  [ALTER_LAYER]: (state, {payload}) => {
    ...
    let alteredLayer, layers, panel, panels
    switch (layerType) {
      case 'data':
  ...
  [SET_WMJSLAYERS]: (state, {payload}) => {
    return {...state, wmjsLayers: payload}
  },
