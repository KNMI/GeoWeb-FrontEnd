 ...
 */
export const isObsOrFcValid = (sigmetProps) => {
  const { validdate, validdate_end: validdateEnd, obs_or_forecast: obsOrForecastObject } = sigmetProps.sigmet;
 ...
 * @returns {boolean} The result
 */
export const isStartValidityTimeValid = (sigmetProps) => {
  const { validdate: validateStart, obs_or_forecast: obsOrForecastObject } = sigmetProps.sigmet;
  const { maxHoursInAdvance } = sigmetProps;
  const isObserved = obsOrForecastObject ? obsOrForecastObject.obs : null;
  if (!validateStart) {
    return false;
  }
 ...
 * @returns {boolean} The result
 */
export const isEndValidityTimeValid = (sigmetProps) => {
  const { validdate: validateStart, validdate_end: validdateEnd } = sigmetProps.sigmet;
  const { maxHoursDuration } = sigmetProps;
