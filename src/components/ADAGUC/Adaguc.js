    ...
    if (currDataLayers.length !== nextDataLayers.length) {
      ...
      for (let i = 0; i < currDataLayers.length; ++i) {
        if (currDataLayers[i].service !== nextDataLayers[i].service ||
            ...
            // currDataLayers[i].dimensions.filter((dim) => !dim.name.includes('time')) !== nextDataLayers[i].dimensions.filter((dim) => !dim.name.includes('time')) || /* TODO: WHAT ARE WE TRYING TO ACHIEVE */
