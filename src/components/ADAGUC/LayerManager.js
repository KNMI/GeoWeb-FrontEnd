...
class LayerName extends Component {
  ...
  togglePopover () {
    ...
    if (!this.state.layers || this.state.layer !== this.props.layer) {
      ...
      let srv = WMJSgetServiceFromStore(this.props.layer.service)
...
class LayerStyle extends Component {
  ...
  alterLayer (e, wantedStyle) {
    const indexInLayerList = e.currentTarget.id
    this.props.dispatch(this.props.layerActions.alterLayer({index: indexInLayerList, layerType: this.props.target.includes('data') ? 'data' : 'base', fieldsNewValuesObj: {style: wantedStyle.name, styleTitle: wantedStyle.title}, activeMapId: this.props.activeMapId}))
    this.setState({popoverOpen: false})
  }
        ...
        return (
          <Row className='layerinfo' key={'base' + i}>
            ...
            <LayerName i={i} color='success' editable name={this.getBaseLayerName(layer)}
              target={'baselayer' + i} layer={layer} dispatch={this.props.dispatch} layerActions={this.props.layerActions} placement='top' />
            <Col />
            <Col />
            <Col />
          </Row>
      ...
      return layers.map((layer, i) => {
        ...
        return (
          <Row className='layerinfo' key={'lgi' + i} style={{marginBottom: '0.1rem'}}>
            ...
            <LayerStyle color='info' editable activeMapId={activeMapId} style={layer.currentStyle} layer={layer} target={'datalayerstyle' + i} i={i} dispatch={dispatch} layerActions={this.props.layerActions} />
