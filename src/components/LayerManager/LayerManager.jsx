...
export default class LayerManager extends PureComponent {
  render() {
    const { dispatch, panelsActions } = this.props;
    ...
    return (
      <Col xs='auto' className={'LayerManager'} style={{ minWidth: '42rem', flexDirection: 'column' }}>
        <Layers dispatch={dispatch} panelsActions={panelsActions} role='overlays' data={overLayers} color='danger' />
        <Layers dispatch={dispatch} panelsActions={panelsActions} role='datalayers' data={dataLayers} color='info' />
        <Layers dispatch={dispatch} panelsActions={panelsActions} role='maplayers' data={baseLayers} color='success' />
