...
function plotWindbarbs(ctx,PSounding,ddSounding,ffSounding,color,imgx,imgy){
	for (var i=0; i<PSounding.length; i=i+2){
		if ((i/2)%2 == 0){plotTemp=309.15;}
		else{plotTemp=311.70;}
...
function pres2fl(pres,pressfc) {
  ...
  if (pres>22632.04) 
    return(1/GAMMA*(T0-T0/Math.exp(-GAMMA*R/g*Math.log(pres/pressfc)))/30.48);
  else
    return( (11000 - R*T1/g*Math.log(pres/22632.04))/30.48);
}
...
function calcCAPES(PCCL,PEL,TCCL,TdCCLinCelsius,TELinCelsius,PSounding,TSounding,TdSounding,TwSounding,imgx,imgy){
	...
	if(muCAPE>Tmucape){Tmucin=muCIN;Tmucape=muCAPE;Tmucapelvl=n;TindexAbvCCL_MU=indexAbvCCL_MU;TPCCL_MU=PCCL_MU;muPrclBttm=muParcelBottom;muPrclTp=muParcelTop;}
