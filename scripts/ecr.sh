...
create_repository() {
  ...
  if ! aws ecr describe-repositories --region $AWS_REGION --repository-name $REPO_NAMESPACE/$REPO; then
    ...
    aws ecr set-repository-policy --repository-name $REPO_NAMESPACE/$REPO --region $AWS_REGION --policy-text "{\"Version\": \"2008-10-17\",\"Statement\": [{\"Sid\": \"AllowImageDownload\",\"Effect\": \"Allow\",\"Principal\": {\"AWS\": [\"arn:aws:iam::$DTA_ACCOUNT_ID:root\",\"arn:aws:iam::$PRD_ACCOUNT_ID:root\"]},\"Action\": [\"ecr:GetDownloadUrlForLayer\",\"ecr:BatchGetImage\",\"ecr:BatchCheckLayerAvailability\"]}]}"
  else
